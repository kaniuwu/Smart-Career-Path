...
export default function Quiz() {
  ...
  // --- MODIFIED FUNCTION TO SAVE CAREER PATH ---
  const handleSelectPath = async (path) => {
    try {
      // 1. Get user info from localStorage (saved during registration)
      const userInfo = JSON.parse(localStorage.getItem('userInfo'));

      // If user info or token is missing, they can't proceed
      if (!userInfo || !userInfo.token) {
        showToast('You must be logged in to save a career path.', 'error');
        navigate('/login');
        return;
      }
      ...
      const dataToUpdate = {
        userId: userInfo._id, // The ID of the user to update
        careerPath: path      // The chosen path (e.g., "placements")
      };

      // 3. Send a PUT request to the backend endpoint
      await axios.put('http://localhost:5000/api/users/career-path', dataToUpdate);

      showToast(`Career path "${careerPaths[path].title}" selected!`, 'success');
      navigate('/dashboard');

    } catch (error) {
      const message = error.response?.data?.message || "Failed to save career path.";
            ...
            <div className="path-selection">
              ...
              <div className="path-buttons">
                {Object.entries(careerPaths).map(([key, path]) => (
                  <button
                    key={key}
                    onClick={() => handleSelectPath(key)} // This now calls the async function
  ...
  // The rest of the component (the quiz part) remains the same
  return (
    <div className="auth-container">
